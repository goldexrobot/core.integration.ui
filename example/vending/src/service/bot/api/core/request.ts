 ...
 * @throws any
 */
export const request = <T>(config: OpenAPIConfig, options: ApiRequestOptions): Promise<ApiResponse<T>> => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (response.status != 200) {
                const known = options?.errors || {};
                if (known[response.status]) {
                    resolve(new ApiResponse<T>({ err: new ApiError(response.status, known[response.status]) }));
                    return;
                }
                throw new Error(`unexpected status ${response.status}: ${response.statusText}`);
            }

            resolve(new ApiResponse<T>({ result: responseBody }));
        } catch (error) {
            reject(error);
        }
    });
